...
def summarize_text(text):
    ...
    if len(tokenized_text["input_ids"][0]) > max_input_length:
        text = summarizer.tokenizer.decode(tokenized_text["input_ids"][0][:max_input_length], skip_special_tokens=True)
...
def main():
    ...
    if uploaded_file:
        ...
        if not document_text:  # If PDF text extraction fails, try OCR
            ocr_text = extract_text_with_ocr(uploaded_file.name)

        combined_text = document_text if document_text else ocr_text
        summary_text = summarize_text(combined_text)

        st.sidebar.success("✅ Document Processed Successfully!")
        st.sidebar.text_area("Extracted Text", combined_text, height=150)
        st.sidebar.text_area("Summarized Text", summary_text, height=100)
        ...
        if vectorstore and llm:
            try:
                with st.spinner("Processing your query... ⚖️"):
                    ...
                    combined_query = f"{summary_text}\n\nUser Query: {user_query}" if summary_text else user_query
